...
int main(int argc, char * argv[]) try
{
    ...
    // std::cout << camera_matrix_1.at<double>(1,2) << ',' << camera_matrix_2.at<double>(1,2) << std::endl;
    // float y_shift = 20; //camera_matrix_1.at<double>(1,2) - camera_matrix_2.at<double>(1,2);
    // camera_matrix_2.at<double>(1,2) -= y_shift;
    // corners_selection_2 = corners_selection_2 - cv::Scalar_<float>(0,y_shift);
    ...
    // camera_matrix_1 = cv::getDefaultNewCameraMatrix(camera_matrix_1, cv::Size(1280,720), true);
    // camera_matrix_2 = cv::getDefaultNewCameraMatrix(camera_matrix_2, cv::Size(1280,720), true);
    // camera_matrix_2 = cv::getOptimalNewCameraMatrix(camera_matrix_2, dist_coeffs_2, cv::Size(1280,720), 1, cv::Size(), (cv::Rect *)0,  true);
    ...
    double rms = cv::stereoCalibrate(object_points,
                                     ...
                                     R, T, E, F,
                                     cv::CALIB_FIX_INTRINSIC, cv::TermCriteria(cv::TermCriteria::MAX_ITER+cv::TermCriteria::EPS, 30, 1e-1));
        // double rms = cv::stereoCalibrate(object_points,
        //                              image_points_2, image_points_1, 
        //                              camera_matrix_2, dist_coeffs_2, 
        //                              camera_matrix_1, dist_coeffs_1,
        //                              cv::Size(1280,720),
        //                              R, T, E, F,
        //                              cv::CALIB_USE_INTRINSIC_GUESS + cv::CALIB_FIX_FOCAL_LENGTH + cv::CALIB_FIX_ASPECT_RATIO + cv::CALIB_FIX_K1 +  + cv::CALIB_FIX_K2 +  + cv::CALIB_FIX_K3,
    ...
    cv::Mat R1z,R2z,P1z,P2z,Qz;            std::cout << "corners 1" << std::endl;
    ...
    while (true)
    {
        // for (int i = 0; i < frames_selection_1.size(); i++)
        for (int i = 0; i < frames_all_1.size(); i++)
        {
            ...
            // // cv::Mat img_2_aux = uls::ThermalFrame(fs::path(frames_all_2[i]), cv::Size(1280,720)).mat();
            // // cv::Mat img_2;
            // // cv::copyMakeBorder(img_2_aux(cv::Rect(0,((int) y_shift),img_2_aux.cols, img_2_aux.rows-((int) y_shift))), img_2, 0, ((int) y_shift), 0, 0, cv::BORDER_CONSTANT);     
        
            // cv::cvtColor(img_1, img_1, cv::COLOR_GRAY2BGR);
            // cv::cvtColor(img_2, img_2, cv::COLOR_GRAY2BGR);

            // cv::Mat a = corners_selection_1.row(i);//.reshape(1,9*6);
            // cv::Mat b = corners_selection_2.row(i);//.reshape(1,9*6);
            // cv::drawChessboardCorners(img_1, cv::Size(9,6), a, true);
            // cv::drawChessboardCorners(img_2, cv::Size(9,6), b, true);

            // cv::Mat rvec, tvec;
            // cv::solvePnP(object_points[0], corners_selection_1.row(i), camera_matrix_1, dist_coeffs_1, rvec, tvec);

            // std::vector<cv::Mat> tiling = {img_1, img_2};
            // cv::Mat viewer_img;
            // uls::tile(tiling, 640, 720, 1, 2, viewer_img);
            ...
            // cv::multiply(tmp_1, cv::Scalar(0,1,0), tmp_1);
            // cv::multiply(tmp_2, cv::Scalar(0,0,1), tmp_2);
            // cv::addWeighted(tmp_1, 1, tmp_2, 1, 0.0, tiling[3]);
            // cv::remap(img_2, tmp_2, mapi1_2, mapi2_2, cv::INTER_LINEAR);
            cv::cvtColor(tmp_2, tiling[3], cv::COLOR_GRAY2BGR);
            ...
            cv::multiply(tmp_1, cv::Scalar(0,0,1), tmp_1);
            cv::multiply(tmp_2, cv::Scalar(0,1,0), tmp_2);
            cv::addWeighted(tmp_1, 1, tmp_2, 1, 0.0, tiling[5]);

            // cv::Mat rvec_1, tvec_1, rvec_2, tvec_2;
            // cv::solvePnP(object_points[0], corners_selection_1.row(i), camera_matrix_1, dist_coeffs_1, rvec_1, tvec_1);
            // cv::solvePnP(object_points[0], corners_selection_2.row(i), camera_matrix_2, dist_coeffs_2, rvec_2, tvec_2);
            // cv::Mat rvec_2_tmp = R * rvec_2 + T;
            // std::cout << rvec_1 << std::endl;
            // std::cout << rvec_2 << std::endl;
            // std::cout << R*rvec_2+T << std::endl;
            // std::cout << R*tvec_2+T << std::endl;
            // std::cout << R*rvec_1+T << std::endl;
            // std::cout << R*tvec_1+T << std::endl;

            cv::Mat camera_matrix = cv::Mat::eye(3, 3, CV_64F);
            cv::Mat dist_coeffs = cv::Mat::zeros(8, 1, CV_64F);
            // cv::Mat image_points_aux_1, image_points_aux_2;
            // cv::projectPoints(object_points[0], R*rvec_2+T, R*tvec_2, camera_matrix_1, dist_coeffs_1, image_points_aux_1);
            // cv::projectPoints(object_points[0], R*rvec_1+T, R*tvec_1+T, camera_matrix_2, dist_coeffs_2, image_points_aux_2);
            cv::Mat aa, bb, cc, dd, ee, ff;
            cv::Mat P1r, P1cm, P1t, P2r, P2cm, P2t;
            cv::decomposeProjectionMatrix(P1, P1cm, P1r, P1t);
            std::cout << "Matrices 1" << std::endl;
            std::cout << camera_matrix_1 << std::endl;
            std::cout << P1 << std::endl;
            std::cout << P1r << std::endl;
            std::cout << P1cm << std::endl;
            std::cout << P1t << std::endl;
            cv::decomposeProjectionMatrix(P2, P2cm, P2r, P2t);
            std::cout << "Matrices 2" << std::endl;
            std::cout << camera_matrix_2 << std::endl;
            std::cout << P2 << std::endl;
            std::cout << P2r << std::endl;
            std::cout << P2cm << std::endl;
            std::cout << P2t << std::endl;
            std::cout << "---" << std::endl;
            // cv::undistortPoints(corners_selection_1.row(i), aa, camera_matrix_1, dist_coeffs_1, R1);
            // cv::undistortPoints(aa, bb, camera_matrix, dist_coeffs, R1.inv(), camera_matrix_1);
            // std::cout << corners_selection_1.row(i) << std::endl;
            // BEST 2nd
            // // ---
            cv::Mat camera_matrix_1def = cv::getDefaultNewCameraMatrix(camera_matrix_1, cv::Size(1280,720+abs(y_shift_1)), true);
            cv::Mat camera_matrix_2def = cv::getDefaultNewCameraMatrix(camera_matrix_2, cv::Size(1280,720+abs(y_shift_1)), true);
            // _undistortPoints(corners_selection_1.row(i), aa, camera_matrix_1, dist_coeffs_1, R1);
            // // for (int i = 0; i < aa.cols; i++)
            // // {
            // //   aa.at<cv::Point2f>(0,i).x = aa.at<cv::Point2f>(0,i).x - P2t.at<float>(0,0);
            // // }
            // _undistortPoints(aa, ee, camera_matrix, dist_coeffs, R1.inv(), camera_matrix_2def);
            // ---
             // BEST
            double tx, ty, tz, tw;
            tx = P2t.at<double>(0,0);
            ty = P2t.at<double>(1,0);
            tz = P2t.at<double>(2,0);
            tw = P2t.at<double>(3,0);
            _undistortPoints(corners_selection_1.row(i), aa, camera_matrix_1, dist_coeffs_1, R1);
            _undistortPoints(aa-P2t, ee, camera_matrix, dist_coeffs, (R2).inv(), camera_matrix_2def);
            _undistortPoints(corners_selection_1.row(i), aa, camera_matrix_1, dist_coeffs_1, R1);
            _undistortPoints(aa-cv::Scalar_<double>(tx,ty), ee, camera_matrix, dist_coeffs, (R2).inv(), camera_matrix_2);
            // _undistortPoints(ee, ee, camera_matrix_2, dist_coeffs_2, camera_matrix);
            // std::cout << aa << std::endl;
            // for (int i = 0; i < aa.cols; i++)
            //   std::cout << aa.at<cv::Point2f>(0,i) << std::endl;
            // std::cout << P2t << std::endl;
            // std::cout << aa-P2t << std::endl;
            // for (int i = 0; i < aa.cols; i++)
            // {
            //   cv::Mat aa2 = aa-P2t;
            //   std::cout << (aa2).at<cv::Point2f>(0,i) << std::endl;
            // }
            // ---
            // CHECK: equivalence aa == cc? NO
            // cv::undistortPoints(corners_selection_1.row(i), aa, camera_matrix_1, dist_coeffs_1, R1);
            // cv::undistortPoints(corners_selection_1.row(i), bb, camera_matrix_1, dist_coeffs_1);
            // cv::undistortPoints(bb, cc, camera_matrix, cv::Mat(), R1);
            // std::cout << aa - cc << std::endl;
            // ---
            // CHECK: equivalence aa == cc?
            // cv::undistortPoints(corners_selection_1.row(i), aa, camera_matrix_1, dist_coeffs_1, R1);
            // cv::undistortPoints(corners_selection_1.row(i), bb, camera_matrix_1, dist_coeffs_1);
            // cv::undistortPoints(bb, cc, camera_matrix, dist_coeffs, R1);
            // cv::undistortPoints(cc, dd, camera_matrix, dist_coeffs, R1.inv());
            // std::cout << aa - cc << std::endl;
            // std::cout << aa - dd << std::endl;
            // std::cout << bb - dd  << std::endl;
            // for (int k = 0; k < bb.cols; k++)
            // {
            //   std::cout << bb.at<cv::Point2f>(0,k) << std::endl;
            // }

            // cv::undistortPoints(ee, bb, camera_matrix_2, dist_coeffs_2);
            // cv::undistortPoints(bb, cc, camera_matrix_2, dist_coeffs_2, R2);
            // std::cout << corners_selection_1.row(i) << std::endl;
            // std::cout << ee << std::endl;

            // cv::undistortPoints(aa, bb, camera_matrix, dist_coeffs, R2*R1.inv(), camera_matrix_2);
            // std::cout << bb << std::endl;
            // std::cout << corners_selection_1.row(i) - aux2 << std::endl;
            // cv::drawChessboardCorners(tiling[0], cv::Size(9,6), image_points_aux_1, true);
            // cv::drawChessboardCorners(tiling[3], cv::Size(9,6), bb, true);
            // cv::undistortPoints(corners_selection_2.row(i), cc, camera_matrix_2, dist_coeffs_2, camera_matrix_2);
            // cv::drawChessboardCorners(tiling[3], cv::Size(9,6), cc, true);
            // std::cout << cc << std::endl;
            // cv::drawChessboardCorners(tiling[3], cv::Size(9,6), dd, false);
            cv::drawChessboardCorners(tiling[3], cv::Size(9,6), ee, true);
            // cv::drawChessboardCorners(tiling[3], cv::Size(9,6), ee, false);
            // cv::Mat dev;
            // cv::absdiff(cc, ee, dev);
            // std::cout << "ABSDIFF: " << cv::sum(dev).val[0] << std::endl;
            cv::Mat viewer_img;
            ...
            // cv::computeCorrespondEpilines(corners_selection_1.row(i), 1, F, lines_2);
            // cv::computeCorrespondEpilines(corners_selection_2.row(i), 2, F, lines_1);

            // std::cout << "corners 1" << std::endl;
            // std::cout << corners_selection_1.row(i) << std::endl;
            // std::cout << "corners 2" << std::endl;
            // std::cout << corners_selection_2.row(i) << std::endl;
            // cv::Mat img_epi = drawEpipolarLines<float,float>(F, cv::Mat::zeros(cv::Size(1280,720), CV_8UC3), cv::Mat::zeros(cv::Size(1280,720), CV_8UC3), corners_selection_1.row(i), corners_selection_2.row(i));
                // draw the left points corresponding epipolar lines in right image
            
            // std::vector<cv::Point2f> points1 (9*6);
            // std::vector<cv::Point2f> points2 (9*6);
            // for (int k = 0; k < points1.size(); k++)
            // {
            //   points1[k] = corners_selection_1.at<cv::Point2f>(i,k) + cv::Point2f(-100,0);
            //   points2[k] = corners_selection_2.at<cv::Point2f>(i,k) + cv::Point2f(100,0);
            // }

            // cv::cvtColor(img_1, img_1, cv::COLOR_GRAY2BGR);
            // cv::cvtColor(img_2, img_2, cv::COLOR_GRAY2BGR);
            // cv::Mat a = corners_selection_2.row(i);//.reshape(1,9*6);
            // cv::Mat b = corners_selection_1.row(i);//.reshape(1,9*6);
            // cv::drawChessboardCorners(img_1, cv::Size(9,6), a, true);
            // cv::drawChessboardCorners(img_2, cv::Size(9,6), b, true);
            // // F = cv::findFundamentalMat(points1, // points in first image
            // //                           points2, // points in second image
            // //     CV_FM_RANSAC); // 7-point method
            // std::cout << a << std::endl;
            // std::cout << b << std::endl;
            // std::cout << a-b << std::endl;
            // cv::Mat image1 = cv::Mat::zeros(cv::Size(1280,720), CV_8UC3);
            // cv::Mat image2 = cv::Mat::zeros(cv::Size(1280,720), CV_8UC3);
            // std::vector<cv::Vec3f> lines1;
            // cv::computeCorrespondEpilines(points1, // image points
            ...
            //             cv::Point(img_2.cols, -((*it)[2] + (*it)[0] * img_2.cols) / (*it)[1]),
            ...
            //     // draw the epipolar line between first and last column
            //     cv::line(img_1, cv::Point(0, -(*it)[2] / (*it)[1]),
            //             cv::Point(img_1.cols, -((*it)[2] + (*it)[0] * img_1.cols) / (*it)[1]),
            //             cv::Scalar(255, 255, 255));
            // }
            // cv::Mat img_epi;
            // std::vector<cv::Mat> tiling = {img_1, img_2};
            // uls::tile(tiling, 640, 720, 1, 2, img_epi);
            // // cv::resize(img_epi, img_epi, cv::Size(640,180));
            // cv::imshow("Viewer", img_epi);
            // cv::waitKey();
        }
    }
